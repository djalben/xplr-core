...
func GetAdminChatIDs() ([]int64, error) {
	if GlobalDB == nil {
		log.Println("[GetAdminChatIDs] ❌ GlobalDB is nil!")
	...
	if err != nil {
		log.Printf("[GetAdminChatIDs] ❌ Query error: %v", err)
		return nil, err
	}
	defer rows.Close()
	...
	log.Printf("[GetAdminChatIDs] Returning %d admin TG IDs: %v", len(ids), ids)
	return ids, rows.Err()
}
