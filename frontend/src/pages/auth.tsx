...
export const AuthPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('[Auth] Response:', { token: res.token ? 'yes' : 'NO', requires_2fa: res.requires_2fa });
      ...
      // Register success (no token): ask user to verify by link
      if (mode === 'register') {
        setInfo(res.message || 'Регистрация успешна. Подтвердите email по ссылке из письма, затем войдите.');
        setMode('login');
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-transparent flex items-center justify-center p-4 overflow-hidden relative z-2">
      ...
      <div className="relative z-10 w-full max-w-md animate-fade-in-up">
        ...
        {/* Info banner (e.g. registration success) */}
        {info && (
          <div className="mb-4 p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm text-center flex items-center justify-center gap-2">
            <Check className="w-4 h-4" />
            {info}
