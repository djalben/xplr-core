...
const ESIMActivationModal = ({
  ...
  result: ESIMOrderResult; planName: string; priceUsd: string; onClose: () => void;
}) => {
  ...
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(result.qrData || result.lpa || '')}`;
  const lpaString = result.lpa || result.qrData || '';
  const smdp = result.smdp || (lpaString.includes('$') ? lpaString.split('$')[1] : '');
  const matchingId = result.matchingId || (lpaString.includes('$') ? lpaString.split('$')[2] : '');
      ...
      <div className="relative z-10 w-full max-w-lg max-h-[92vh] overflow-y-auto rounded-2xl bg-[#0F1629] border border-white/[0.08] shadow-2xl animate-scale-in" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 sm:p-8">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-white/40">{planName} — ${priceUsd}</p>
          </div>
...
export const StorePage = () => {
  ...
  const [esimResult, setEsimResult] = useState<{ result: ESIMOrderResult; planName: string; priceUsd: string } | null>(null);
  ...
  useEffect(() => { if (selectedCountry) loadPlans(selectedCountry.countryCode); }, [selectedCountry, loadPlans]);
            ...
            {destsLoading ? (
              ...
              <div className="rounded-2xl bg-white/[0.02] border border-white/[0.05] overflow-hidden">
                {destinations.map((dest, i) => (
                  <button key={dest.countryCode} onClick={() => setSelectedCountry(dest)}
                    ...
                    <CountryFlag code={dest.countryCode} size={32} />
                    <span className="text-sm sm:text-[15px] font-medium text-white flex-1 truncate">{dest.countryName}</span>
                    <span className="text-xs sm:text-sm text-white/30 flex-shrink-0 tabular-nums">{dest.planCount} тарифа</span>
        ...
        {storeView === 'esim' && selectedCountry && (
          <div className="max-w-2xl mx-auto stagger-fade-in">
            <div className="flex items-center gap-4 mb-6 sm:mb-8">
              ...
              <CountryFlag code={selectedCountry.countryCode} size={36} />
              <div>
                <h1 className="text-xl sm:text-2xl font-bold text-white">{selectedCountry.countryName}</h1>
            ...
            {plansLoading ? (
              ...
              <div className="rounded-2xl bg-white/[0.02] border border-white/[0.05] overflow-hidden">
                {plans.map((plan, idx) => (
                  <div key={plan.planId} className={`flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-3.5 sm:py-4 ${plan.inStock ? '' : 'opacity-40'} ${idx < plans.length - 1 ? 'border-b border-white/[0.05]' : ''}`}>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm sm:text-[15px] font-medium text-white truncate">{plan.name}</p>
                      <p className="text-xs text-white/35 mt-0.5">{plan.dataGb} GB / {plan.validityDays} дней</p>
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0">
                      <div className="text-right">
                        <span className="text-sm sm:text-[15px] font-bold gradient-text tabular-nums">${plan.priceUsd}</span>
                        {Number(plan.oldPrice) > 0 && Number(plan.oldPrice) > Number(plan.priceUsd) && <span className="block text-[10px] text-white/20 line-through">${plan.oldPrice}</span>}
                      </div>
                      {plan.inStock ? (
              ...
              <div className="space-y-4">
                {Object.entries(groupedDigital).map(([brand, items]) => (
                  <div key={brand} className="rounded-2xl bg-white/[0.02] border border-white/[0.05] overflow-hidden">
                    {items.map((product, i) => {
                      const BrandIcon = brandIconForProduct(product.name, product.externalId);
                      return (
                        <div key={product.id} className={`flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-3.5 sm:py-4 ${product.inStock ? '' : 'opacity-40'} ${i !== items.length - 1 ? 'border-b border-white/[0.05]' : ''}`}>
                          ...
                          <div className="flex items-center gap-3 flex-shrink-0">
                            <span className="text-sm sm:text-[15px] font-bold gradient-text tabular-nums">${product.priceUsd}</span>
                            {product.inStock ? (
            ...
            <div className="mb-6 rounded-2xl border border-white/[0.08] bg-white/[0.02] overflow-hidden">
              <div className="p-5 sm:p-6">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-white">Быстрый старт</h3>
                    <p className="text-xs text-white/35">Три шага до защищённого соединения</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
                  {[
                    { step: '1', title: 'Скачайте', desc: 'Установите приложение для вашего устройства' },
                    { step: '2', title: 'Купите тариф', desc: 'Выберите план и скопируйте ключ доступа' },
                    { step: '3', title: 'Подключитесь', desc: 'Добавьте ключ через кнопку «+» или «Импорт»' },
                  ].map(s => (
                    <div key={s.step} className="flex items-start gap-3 p-3.5 rounded-xl bg-white/[0.03] border border-white/[0.05]">
                      <div className="w-7 h-7 rounded-lg bg-[#818CF8]/10 flex items-center justify-center flex-shrink-0">
                        <span className="text-xs font-bold text-[#818CF8]">{s.step}</span>
                      </div>
                      <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  >
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#3DDC84]/20 to-[#3DDC84]/5 border border-[#3DDC84]/20 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                      <svg className="w-5 h-5 text-[#3DDC84]" viewBox="0 0 24 24" fill="currentColor"><path d="M17.523 2.226a.75.75 0 0 0-1.046 0L14.1 4.602a8.523 8.523 0 0 0-4.2 0L7.523 2.226a.75.75 0 1 0-1.046 1.078L8.3 5.126A8.5 8.5 0 0 0 3.5 12.5V14h17v-1.5a8.5 8.5 0 0 0-4.8-7.374l1.823-1.822a.75.75 0 0 0 0-1.078ZM8.5 11a1 1 0 1 1 0-2 1 1 0 0 1 0 2Zm7 0a1 1 0 1 1 0-2 1 1 0 0 1 0 2ZM3.5 15.5h17V19a3 3 0 0 1-3 3h-11a3 3 0 0 1-3-3v-3.5Z"/></svg>
                    </div>
                  ...
                  <a
                    ...
                    className="flex items-center gap-4 px-5 py-4 rounded-xl bg-white/[0.04] border border-white/[0.08] hover:bg-white/[0.08] hover:border-white/[0.15] transition-all duration-200 active:scale-[0.98] group"
                  >
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-white/10 to-white/[0.03] border border-white/10 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                      <svg className="w-5 h-5 text-white/80" viewBox="0 0 24 24" fill="currentColor"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11Z"/></svg>
                    </div>
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-white">iOS (iPhone / iPad)</p>
                      <p className="text-[11px] text-white/35 mt-0.5">Happ Proxy (App Store)</p>
                    </div>
                    <Download className="w-4 h-4 text-white/25 ml-auto shrink-0 group-hover:text-white/50 transition-colors" />
                  </a>
                </div>
                  ...
                  return (
                    <div key={product.id} className="group rounded-2xl overflow-hidden bg-white/[0.02] border border-white/[0.06] hover:border-[#818CF8]/40 hover:bg-white/[0.05] hover:shadow-[0_0_24px_rgba(129,140,248,0.12)] transition-all duration-300">
                      <div className="p-5 sm:p-6">
                        ...
                        >
                          Подключить за ${product.priceUsd}
                        </button>
                      </div>
                    </div>
                  );
