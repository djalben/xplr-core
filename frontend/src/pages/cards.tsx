...
const CardIssueModal = ({ 
  ...
  onClose: () => void;
  onIssue?: (cardType: 'subscriptions' | 'travel' | 'premium', priceUsd: number, currency: 'USD' | 'EUR') => void;
  isIssuing?: boolean;
          ...
          <button 
            onClick={() => {
              if (onIssue) {
                ...
                onIssue(card.type as 'subscriptions' | 'travel' | 'premium', priceNum, selectedCurrency);
              } else {
                onClose();
              }
            }}
...
export const CardsPage = () => {
  ...
  const mapBackendCard = (bc: BackendCard, details?: { full_number: string; cvv: string; expiry: string }): PersonalCard => {
    const slug = bc.card_type || 'subscriptions';
    ...
    const isEur = (bc.currency || '').toUpperCase() === 'EUR';
    return {
      id: String(bc.id),
      type: cardType,
      ...
      cvv: details?.cvv || '***',
      balance: parseFloat(bc.balance || '0'),
      currency: isEur ? '€' : '$',
  ...
  const handleIssueCard = async (
    ...
    setIsIssuing(true);
    try {
      const result = await issuePersonalCard(cardType, priceUsd, currency);
  ...
  const handleCloseCard = async (card: PersonalCard) => {
    try {
      await updateCardStatus(card.id, 'CLOSED');
